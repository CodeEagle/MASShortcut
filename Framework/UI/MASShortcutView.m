...
- (void)commonInit {
  ...
  _shortcutCell.font =
      [[NSFontManager sharedFontManager] convertFont:_shortcutCell.font
                                              toSize:MASButtonFontSize];
  ...
  if (_recording == NO && (&NSAccessibilityPriorityKey != NULL)) {
    NSString *msg =
        _shortcutValue
            ...
            : MASLocalizedString(@"Shortcut cleared",
...
- (void)drawRect:(CGRect)dirtyRect {
  if (self.shortcutValue) {
    ...
    NSString *title =
        (self.recording
             ? (_hinting
                    ? MASLocalizedString(@"Use Old Shortcut",
                                         @"Cancel action button for non-empty "
                                         @"shortcut in recording state")
                    : (self.shortcutPlaceholder.length > 0
                           ? self.shortcutPlaceholder
                           : MASLocalizedString(@"Type New Shortcut",
                                                @"Non-empty shortcut button in "
                                                @"recording state")))
         : _shortcutValue ? _shortcutValue.description
                          : @"");
    [self drawInRect:shortcutRect
           withTitle:title
           alignment:NSTextAlignmentCenter
               state:self.isRecording ? NSControlStateValueOn
                                      : NSControlStateValueOff];
  } else {
    if (self.recording) {
      [self drawInRect:self.bounds
             withTitle:NSStringFromMASKeyCode(kMASShortcutGlyphEscape)
             alignment:NSTextAlignmentRight
                 state:NSControlStateValueOff];
      ...
      [self getShortcutRect:&shortcutRect hintRect:NULL];
      NSString *title =
          (_hinting ? MASLocalizedString(
                          @"Cancel", @"Cancel action button in recording state")
                    : (self.shortcutPlaceholder.length > 0
                           ? self.shortcutPlaceholder
                           : MASLocalizedString(
                                 @"Type Shortcut",
...
- (NSSize)intrinsicContentSize {
  ...
  // Use a "fake" value for width.  Since determining the actual width requires
  // information that is not determined until drawRect: is called, it doesn't
  // seem feasible to properly calculate the intrinsic size without refactoring
  // the code.  That would give better results, however.

  // 120 is an arbitrary number that seems to be wide enough for English
  // localization.  This may need to be adjusted for other locales/languages.

  // NOTE:  Simply returning cellSize results in a display that is sometimes
  // correct and sometimes not, and changes based on whether the mouse is
  // hovering or not.
...
- (void)getShortcutRect:(CGRect *)shortcutRectRef
               hintRect:(CGRect *)hintRectRef {
  ...
  case MASShortcutViewStyleRounded:
    ...
    break;
  case MASShortcutViewStyleFlat:
    hintButtonWidth -= 8.0 - (_shortcutCell.font.pointSize - MASButtonFontSize);
    break;
  default:
    break;
  }
  CGRectDivide(self.bounds, &hintRect, &shortcutRect, hintButtonWidth,
               CGRectMaxXEdge);
  if (shortcutRectRef)
...
- (BOOL)locationInShortcutRect:(CGPoint)location {
  CGRect shortcutRect;
  [self getShortcutRect:&shortcutRect hintRect:NULL];
  return CGRectContainsPoint(shortcutRect, [self convertPoint:location
                                                     fromView:nil]);
}
...
- (BOOL)locationInHintRect:(CGPoint)location {
  ...
  return CGRectContainsPoint(hintRect, [self convertPoint:location
                                                 fromView:nil]);
}
...
- (void)updateTrackingAreas {
  ...
  _hintArea = [[NSTrackingArea alloc] initWithRect:hintRect
                                           options:options
                                             owner:self
                                          userInfo:nil];
...
- (void)resetToolTips {
  ...
  _shortcutToolTipTag = [self addToolTipRect:shortcutRect
                                       owner:self
                                    userData:kUserDataShortcut];
  _hintToolTipTag = [self addToolTipRect:hintRect
                                   owner:self
...
- (NSString *)view:(NSView *)view
  ...
  if (data == kUserDataShortcut) {
    return MASLocalizedString(@"Click to record new shortcut",
                              @"Tooltip for non-empty shortcut button");
  } else if (data == kUserDataHint) {
    return MASLocalizedString(
        @"Delete shortcut",
    ...
    eventMonitor = [NSEvent
        addLocalMonitorForEventsMatchingMask:eventMask
                                     handler:^(NSEvent *event) {
                                       ...
                                       // If the shortcut is a plain Delete or
                                       // Backspace, clear the current shortcut
                                       // and cancel recording
                                       ...
                                       // If the shortcut is a plain Esc, cancel
                                       // recording
                                       else if (!shortcut.modifierFlags &&
                                                shortcut.keyCode ==
                                                    kVK_Escape) {
                                         weakSelf.recording = NO;
                                         event = nil;
                                       ...
                                       // If the shortcut is Cmd-W or Cmd-Q,
                                       // cancel recording and pass the event
                                       // through
                                       else if ((shortcut.modifierFlags ==
                                         ...
                                         if (shortcut.keyCodeString.length >
                                             0) {
                                           if (!weakSelf.shortcutValidator ||
                                               [weakSelf.shortcutValidator
                                                   isShortcutValid:shortcut]) {
                                             // Verify that shortcut is not used
                                             NSString *explanation = nil;
                                             if ([weakSelf.shortcutValidator
                                                     isShortcutAlreadyTakenBySystem:
                                                         shortcut
                                                                        explanation:
                                                                            &
                                                                            explanation]) {
                                               // Prevent cancel of recording
                                               // when Alert window is key
                                               [weakSelf
                                                   activateResignObserver:NO];
                                               [weakSelf
                                                   activateEventMonitoring:NO];
                                               NSString *format =
                                                   MASLocalizedString(
                                                       @"The key combination "
                                                       @"%@ cannot be used",
                                                       @"Title for alert when "
                                                       @"shortcut is already "
                                                       @"used");
                                               NSAlert *alert =
                                                   [[NSAlert alloc] init];
                                               ...
                                               [alert runModal];
                                               weakSelf.shortcutPlaceholder =
                                                   nil;
                                               [weakSelf
                                                   activateResignObserver:YES];
                                               [weakSelf
                                                   activateEventMonitoring:YES];
                                             } else {
                                               weakSelf.shortcutValue =
                                                   shortcut;
                                               weakSelf.recording = NO;
                                             }
                                           } else {
                                             // Key press with or without SHIFT
                                             // is not valid input
                                             NSBeep();
                                           }
                                         } else {
...
- (void)propagateValue:(id)value forBinding:(NSString *)binding {
  ...
  if (bindingOptions) {
    NSValueTransformer *transformer =
        [bindingOptions valueForKey:NSValueTransformerBindingOption];
    if (!transformer || (id)transformer == [NSNull null]) {
      NSString *transformerName =
          [bindingOptions valueForKey:NSValueTransformerNameBindingOption];
    ...
    if (transformer && (id)transformer != [NSNull null]) {
      if ([[transformer class] allowsReverseTransformation]) {
        ...
        NSLog(@"WARNING: binding \"%@\" has value transformer, but it doesn't "
              @"allow reverse transformations in %s",
...
- (NSString *)accessibilityHelp {
  return MASLocalizedString(
      @"To record a new shortcut, click this button, and then type the"
...
- (NSString *)accessibilityLabel {
  ...
  title = [title
      stringByAppendingFormat:@" %@", MASLocalizedString(@"keyboard shortcut",
                                                         @"VoiceOver title")];
  return title;
}
